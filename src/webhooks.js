...
async function dispatchSingleWebhook(topicId, messageId, url, payload, attempt = 1, existingLogId = null) {
  ...
  if (isSlackCompatible && typeof payload === 'object' && payload !== null) {
    ...
    if (existingLogId) {
      ...
      const { rows } = await pool.query(
        `INSERT INTO webhook_logs 
         ...
         RETURNING id`,
        [topicId, messageId, url, statusCode, JSON.stringify(finalPayload), responsePayload || null, durationMs, success, errorMessage || null, attempt]
      );
      existingLogId = rows[0].id;
    }
  } catch (dbErr) {
  ...
  if (!success && attempt < MAX_ATTEMPTS) {
    ...
    setTimeout(() => {
      dispatchSingleWebhook(topicId, messageId, url, finalPayload, attempt + 1, existingLogId);
    }, delay);
  }
}
